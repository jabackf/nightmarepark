...
void UIPlotPixel(int, int, unsigned short int);
void DMAClearScreen(u16);
...
void mosaic_in(s16 new_mode)
{
	u16 mos=0;
	while (mos<=7)
	{
		mos++;
		REG_MOSAIC = SetMosaic(mos,mos,0,0);
		WaitTime(0,30);
	}
	if (new_mode>=0) GameMode=(u16)new_mode;
}
void mosaic_out(s16 new_mode)
{
	u16 mos=7;
	while (mos>0)
	{
...
////////////////////////////////////////////
//This fuction plots a pixel on the screen//
////////////////////////////////////////////

void UIPlotPixel(int x,int y, unsigned short int c)
{
